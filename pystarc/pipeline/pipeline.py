...
from pystarc.pipeline.prepare_bd_surface import read_pqr, write_pqr, centre_at_origin, inject_gho as _inject_gho
from pystarc.pathways.reaction_interface import ReactionInterface, PathwaySet, ReactionCriteria, ContactPair
...
def run(cfg: PySTARCConfig):
    ...
    rxn_r = cfg.bd_milestone_radius_inner if cfg.bd_milestone_radius_inner > 0             else cfg.bd_milestone_radius
    ...
    # Step 5: Geometry 
    geom    = compute_geometry(receptor_pqr, ligand_pqr,
    ...
    rxn_result = auto_detect_reactions(geom, cfg.ghost_atoms, cfg.rxns_xml,
                                       bd_milestone_radius=cfg.bd_milestone_radius,
                                       bd_milestone_radius_inner=cfg.bd_milestone_radius_inner)
    rxn_stages, rxn_n_needed = rxn_result if isinstance(rxn_result, tuple) else (rxn_result, -1)
    ...
    print(f"  r_hydro: receptor={geom.receptor.hydrodynamic_r:.3f} Å  "
    ...
    for stage_idx, stage_pairs in enumerate(rxn_stages):
        ...
        reactions.append(ReactionInterface(
            name     = f"stage_{stage_idx}",
            criteria = crit,
        ))
    ...
    if cfg.gpu and engine.backend == 'cupy':
        ...
        def _load_dx(prefix, suffix):
            return [DXGrid.from_file(p)
                    for p in sorted(W.glob(f"{prefix}[0-9]{suffix}"))]
        ...
        gpu_sim = GPUBatchSimulator(mol_rec, mol_lig, mob,
    ...
    print(f"  Hydro radii from XML           (rec={_rh_r:.4f}, lig={_rh_l:.4f})" 
          if _rh_r > 0 else "  Hydro radii from MC")
    ...
    if cfg.lj_forces:
        ...
        print(f"  LJ forces disabled (use <lj_forces>true</lj_forces> for tight contact)")
    print(f"  Born both directions            (alpha={_desolv:.8f})" 
          if _desolv > 0 else "  Born desolvation disabled (alpha=0)")
    print(f"  Variable dt (pair_dt + force_dt + edge_dt)")
    print(f"  minimum_core_dt = {_mcd:.1f} ps" if _mcd > 0 else 
          "  No minimum_core_dt floor")
