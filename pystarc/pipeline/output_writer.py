...
def write_all(work_dir: Path, result, sim_data: Dict[str, Any],
              output_cfg, k_b: float, D_rel: float,
    ...
    if output_cfg.trajectories_csv and 'outcome' in sim_data:
        ...
        with open(p, 'w') as f:
            f.write('traj_id,outcome,n_steps,start_x,start_y,start_z,'
            ...
            for i in range(N):
                ...
                f.write(f'{i},{ostr},{int(n_steps[i])},'
        ...
        if len(epos) > 0:
            ...
            with open(p, 'w') as f:
                ...
                for i in range(len(epos)):
                    f.write(f'{int(etraj[i])},{int(estep[i])},'
        ...
        if len(npos) > 0:
            ...
            with open(p, 'w') as f:
                ...
                for i in range(len(npos)):
                    f.write(f'{int(ntraj[i])},{ndist[i]:.6f},'
    ...
    if output_cfg.radial_density and 'radial_bins' in sim_data:
        ...
        with open(p, 'w') as f:
            ...
            for i in range(len(counts)):
                ...
                f.write(f'{rc:.4f},{bins[i]:.4f},{bins[i+1]:.4f},'
    ...
    if output_cfg.milestone_flux and 'milestone_radii' in sim_data:
        ...
        with open(p, 'w') as f:
            ...
            for i in range(len(radii)):
                f.write(f'{radii[i]:.4f},{int(fout[i])},{int(fin[i])},'
                        f'{int(fout[i]-fin[i])}\n')
    ...
    if output_cfg.p_commit and 'outcome' in sim_data:
        ...
        for i in range(len(outcome)):
            ...
            if outcome[i] == 1:
                p_react[idx] += 1
        p_commit_arr = np.divide(p_react, n_count,
                                  out=np.zeros_like(p_react),
                                  where=n_count > 0)
        p_path = work_dir / 'p_commit.npz'
        np.savez_compressed(p_path,
            r_bins=bins,
            p_commit=p_commit_arr,
            n_samples=n_count)
    ...
    if output_cfg.transition_matrix and 'trans_matrix' in sim_data:
        ...
        np.savez_compressed(p,
            bins=sim_data['trans_bins'],
            counts=sim_data['trans_matrix'])
        ...
        if len(eq) > 5:
            ...
            with open(p, 'w') as f:
                ...
                for i in range(len(eq)):
                    f.write(f'{int(etraj[i])},{int(labels[i])},{angles[i]:.6f},'
