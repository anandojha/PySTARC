...
class GPUBatchResult:
    ...
    def rate_constant_ci(self, D_rel: float, k_b: float = 0.0,
    ...
    def summary(self, D_rel: float, k_b: float = 0.0,
...
class GPUBatchSimulator:
    ...
    """
    def __init__(self, mol1, mol2, mob, pathway_set, params: NAMParameters,
                 batch_engine):
        if not _CUPY:
        ...
        self._mol2_pos0 = cp.asarray(
            mol2.positions_array() - c0, dtype=cp.float64)    # (N_lig, 3)
        self._charges   = cp.asarray(
            mol2.charges_array(),        dtype=cp.float64)    # (N_lig,)
        ...
        self._k_b          = 0.0    # set in run(): Romberg k_b (encounter rate from Romberg integral)
        ...
        self._rxn_cutoff_min = float(self._rxn_cutoffs.min()) \
            if len(self._rxn_cutoffs) else 5.0
    ...
    def _compute_return_prob(self) -> float:
        ...
        try:
            ...
            self._k_b = 4.0 * math.pi / romberg_integrate(
                intgd_romberg, 0.0, 1.0 / b, tol=1e-8)
            # k_b(q_out) = Romberg integral to escape sphere
            k_q = 4.0 * math.pi / romberg_integrate(
                intgd_romberg, 0.0, 1.0 / q_out, tol=1e-8)
    ...
    def run(self) -> GPUBatchResult:
        ...
        print(f"  GPU Batch BD: {N:,} trajectories x up to "
        ...
        print(f"  Outer propagator (LMZ): return_prob={self._return_prob:.4f}"
        ...
        if _ckpt_path_resume.exists():
            try:
                ...
                print(f"  Resumed from checkpoint: step={_resume_step}, "
        ...
        _enc_pos = []; _enc_q = []; _enc_traj = []; _enc_step = []; _enc_npairs = []
        ...
        _adt_V0       = (float(self.mol1.total_charge()) *
        ...
        if self._rec_gho_pos is not None and len(self._rec_gho_indices) > 0:
            ...
            print(f"  BB D_eff: D_trans={D_t:.5f}, D_rot_rec={D_rot_rec:.6f}, "
                  f"D_rot_lig={D_rot_lig:.6f}")
            print(f"  BB D_eff per pair: min={D_eff_np.min():.5f}, "
        ...
        print(f"  Receptor: Q={float(self.mol1.total_charge()):+.2f} e, "
              f"n_atoms={len(self.mol1.atoms)}")
        print(f"  Ligand:   Q={float(self.mol2.total_charge()):+.2f} e, "
        ...
        print(f"  Mol2 pos0: {_m2p.shape[0]} atoms, "
        ...
        print(f"  Engine has Yukawa fallback: {getattr(self.engine, '_has_yukawa', False)}")
        print(f"  Engine V_factor (Q_rec only): "
              f"{getattr(self.engine, '_V_factor', 'N/A')}")
        print(f"  Engine receptor_charge: "
        ...
        if hasattr(self.engine, '_elec_grids_gpu'):
            ...
            for i, g in enumerate(self.engine._elec_grids_gpu):
                lo_np = g['lo'] if isinstance(g['lo'], np.ndarray) else np.array(g['lo'])
                hi_np = g['hi'] if isinstance(g['hi'], np.ndarray) else np.array(g['hi'])
                print(f"    Grid {i}: nx={g['nx']} ny={g['ny']} nz={g['nz']}  "
        ...
        if self._rec_gho_pos is not None:
            ...
            print(f"  Rec GHO: {_gho.shape[0]} positions, "
        ...
        if self._use_hi:
            ...
            _hi_ainv = (_hi_Df/(_hi_pi6*_hi_a0) + _hi_Df/(_hi_pi6*_hi_a1)) / _hi_Df
        # Convergence milestones & checkpoint setup 
        _conv_interval = getattr(self.params, 'convergence_interval', 10)
        if _conv_interval > 0:
            _conv_milestones = set(int(N * p / 100) for p in range(_conv_interval, 100, _conv_interval))
        else:
        ...
        if _use_overlap:
            ...
            print(f"  Overlap check: enabled (threshold={_overlap_threshold} Å, "
        ...
        for step in range(_resume_step, self.params.max_steps):
            ...
            if step == 0:
                print(f"  Force batch: {_force_batch:,} traj/batch "
            ...
            if self._rec_gho_pos is not None:
                ...
                _gho_pos = cp.einsum('nij,kj->nki',
                                     R, _gho_mol2) + pos_run[:, None, :]
            ...
            if cp.any(at_escape):
                ...
                u_outer  = cp.asarray(
                    rng.random(int(at_escape.sum())), dtype=cp.float64)
            ...
            for _fb0 in range(0, N_sr, _force_batch):
                ...
                _pos_lig_b = cp.einsum('nij,kj->nki',
                    _R_b, self._mol2_pos0) + _cen_b[:, None, :]
                _f_b, _t_b, _ = self.engine(
                    _pos_lig_b, self._charges,
                    R_matrices=_R_b, centroids=_cen_b)
            ...
            if self._rec_gho_pos is not None:
                ...
                _lig_gho_old = cp.einsum('nij,kj->nki',
                    R_sr, _gho_mol2) + cen_sr[:, None, :]
                _rec_gho_bcast = self._rec_gho_pos[None, :, :]
                _old_pair_dists = cp.linalg.norm(
                    _lig_gho_old - _rec_gho_bcast, axis=2)  # (N_sr, n_pairs)
            # Adaptive time step             #
            # 1. pair_dt:
            ...
            if self._use_hi:
                ...
                _rm3    = _rm1 ** 3
                _Di     = -2.0 * _hi_Df * (2.0*_rm1 - (4.0/3.0)*_hi_a2*_rm3) / _hi_pi8
            ...
            omega   = (D_r * dt_arr[:, None] * torques_gpu +
                       sigma_r * noise_r)
            ...
            if _use_overlap and _rec_pos_overlap is not None:
                ...
                for _ov_c0 in range(0, N_sr_ov, _ov_chunk):
                    ...
                    _diff = new_pos_sr[_ov_c0:_ov_c1, None, :] - _rec_pos_overlap[None, :, :]
            ...
            if (_old_pair_dists is not None and len(sr_idx) > 0
                ...
                _gho_mol2 = self._mol2_pos0[self._lig_gho_indices]
                _lig_gho_new = cp.einsum('nij,kj->nki',
                    R_new, _gho_mol2) + pos[sr_idx][:, None, :]
                _rec_gho_bcast = self._rec_gho_pos[None, :, :]
                _new_pair_dists = cp.linalg.norm(
                    _lig_gho_new - _rec_gho_bcast, axis=2)  # (N_sr, n_pairs)
                ...
                if cp.any(_both_above):
                    ...
                    _p_cross = cp.where(_both_above, _p_cross,
                                        cp.zeros_like(_p_cross))
                    # Sample crossing for each pair
                    _u_bb = cp.asarray(rng.random(_p_cross.shape),
                                       dtype=cp.float64)
            ...
            # Radial density histogram
            np.add.at(_rad_counts,
                      np.clip(np.searchsorted(_rad_bins, _cur_r) - 1,
                              0, len(_rad_counts) - 1),
                      1)
            ...
            # Transition matrix
            _old_bin = np.clip(np.searchsorted(_trans_bins, _prev_r_np) - 1,
                              0, _trans_n - 1)
            _new_bin = np.clip(np.searchsorted(_trans_bins, _cur_r) - 1,
                              0, _trans_n - 1)
            for j in range(len(_old_bin)):
                _trans_mat[_old_bin[j], _new_bin[j]] += 1
            # Contact frequency
            if (self._rec_gho_pos is not None and _n_rxn_pairs > 0
                    and len(sr_idx) > 0):
                _gho_mol2_cf = self._mol2_pos0[self._lig_gho_indices]
                _lig_gho_cf = cp.einsum('nij,kj->nki',
                    R_sr, _gho_mol2_cf) + cen_sr[:, None, :]
                ...
                for pi in range(_n_rxn_pairs):
                    ...
                    _contact_counts[pi] += int(
                        (_pdist < self._rxn_cutoffs[pi]).sum())
            ...
            if step > 0 and step % 10000 == 0:
                if done > int(N * 0.95) and done - _stall_done < max(2, int(N * 0.001)):
                    print(f"  Stall detected: {N-done} trajectories trapped "
            ...
            if step % 1000 == 0:
                ...
                print(f"  step {step:6d} | done={done:6d}/{N} | "
                      f"reacted={n_reacted} escaped={n_escaped} | "
                      f"{sps:,.0f} steps/sec")
                print(f"    r: mean={r_mean:.2f} min={r_min:.2f} max={r_max:.2f} Å  "
                      f"(b={r_b:.1f}, esc={r_esc:.1f})")
                print(f"    |F|: mean={f_mag:.6f} max={f_max:.6f} min={f_min:.6f} kBT/Å")
                print(f"    dt: mean={dt_mean:.4f} min={dt_min:.4f} max={dt_max_:.4f} ps")
                print(f"    drift: {drift_mag:.6f} Å  noise(sigma): {noise_mag:.4f} Å  "
                      f"drift/noise={drift_mag/(noise_mag+1e-30):.4f}")
                print(f"    zone: {n_near_b} inner (r<=b) + {n_far} outer (r>b)  "
                ...
                if step <= 2:
                    ...
                    for ti in range(n_show):
                        ...
                        print(f"      traj[{int(sr_idx[ti])}]: pos=({_pos[0]:.4f},"
        ...
        elapsed    = time.time() - t0
        sps        = total_steps / elapsed if elapsed > 0 else 0
        print(f"  BD complete: {n_reacted} reacted, {n_escaped} escaped, "
    ...
    def _apply_rotation_gpu(self, q: "cp.ndarray",
                            omega: "cp.ndarray") -> "cp.ndarray":
