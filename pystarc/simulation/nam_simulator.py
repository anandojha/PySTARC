...
from pystarc.motion.do_bd_step import bd_step, bd_step_wiener, bd_step_adaptive, backstep_due_to_force, escape_radius
...
def _run_trajectory_worker(args):
    ...
    (mol1, mol2, mol2_pos0, mob, pathway_set,
     params, force_fn, rxn_cutoffs, traj_idx) = args
    ...
    for step in range(params.max_steps):
        ...
        if rxn is not None:
            return TrajectoryResult(Fate.REACTED, step, step * params.dt,
                                    float(np.linalg.norm(pos)), rxn)
        # Escape check
        ...
        pos, ori = bd_step_wiener(pos, ori, force, torque, D_t, D_r,
                                   dt, dW_t, dW_r)
        ...
        if params.use_hard_sphere or True:
            ...
            if backstep_due_to_force(force_new, force_old, pos, pos_old,
                                     dt, params.dt_rxn,
                                     radius=mob.radius2):
                ...
                hdt   = dt / 2.0
                # First half-step
                pos, ori = bd_step_wiener(pos_old, ori_old, force_old, torque,
                                           D_t, D_r, hdt, dW_mid_t, dW_mid_r)
                ...
                pos, ori = bd_step_wiener(pos, ori, f2, t2,
                                           D_t2, D_r, hdt, dW_2nd_t, dW_2nd_r)
        ...
        if params.use_hard_sphere:
            ...
            if _check_hard_sphere_overlap(mol1, mol2_scratch):
                ...
                pos, ori = bd_step_wiener(pos_old, ori_old, force_old, torque,
                                           D_t_old, D_r, dt, dW_t2, dW_r2)
    return TrajectoryResult(Fate.MAX_STEPS, params.max_steps,
...
class NAMSimulator:
    ...
    def run_one(self) -> TrajectoryResult:
        ...
        for step in range(self.params.max_steps):
            ...
            if rxn is not None:
                return TrajectoryResult(Fate.REACTED, step, step * self.params.dt,
                                        float(np.linalg.norm(pos)), rxn)
            r = float(np.linalg.norm(pos))
            ...
            if self._outer_prop is not None and r >= QB_FACTOR * self.params.r_start:
                ...
                ori = Quaternion(w=float(ori_arr[0]), x=float(ori_arr[1]),
                                 y=float(ori_arr[2]), z=float(ori_arr[3]))
                if not reached_b:
                    return TrajectoryResult(Fate.ESCAPED, step,
                                            step * self.params.dt, r)
            ...
            if backstep_due_to_force(force_new, force, pos, pos_old,
                                     dt, self.params.dt_rxn,
                                     radius=self.mobility.radius2):
                hdt      = dt / 2.0
                s        = math.sqrt(hdt / 2.0)
                dW_mid_t = 0.5*dW_t + s*self.rng.standard_normal(3)
                dW_mid_r = 0.5*dW_r + s*self.rng.standard_normal(3)
                pos, ori = bd_step_wiener(pos_old, ori_old, force, torque,
                                          D_t, D_r, hdt, dW_mid_t, dW_mid_r)
                ...
                pos, ori = bd_step_wiener(pos, ori, f2, t2, D_t2, D_r,
                                          hdt, dW_t - dW_mid_t, dW_r - dW_mid_r)
            ...
            if self.params.use_hard_sphere:
                ...
                if _check_hard_sphere_overlap(self.mol1, mol2_trial):
                    ...
                    pos, ori = bd_step_wiener(pos_old, ori_old, force, torque,
                                              D_t_old, D_r, dt, dW_t2, dW_r2)
        return TrajectoryResult(Fate.MAX_STEPS, self.params.max_steps,
    ...
    def _run_parallel(self, n: int):
        ...
        args = [
            (self.mol1, self.mol2, self._mol2_pos0, self.mobility,
             self.pathway_set, self.params, self.force_fn,
             self._rxn_cutoffs, i)
            for i in range(n)
        ]
...
class SimulationResult:
    ...
    def rate_constant(self, D_rel: float,
                       k_db: float = 0.0) -> float:
        """
    ...
    def __repr__(self):
        return (f"SimulationResult(N={self.n_trajectories}, "
                f"reacted={self.n_reacted}, escaped={self.n_escaped}, "
...
def _result_summary(self, D_rel: float, k_b: float = 0.0, confidence: float = 0.95) -> str:
