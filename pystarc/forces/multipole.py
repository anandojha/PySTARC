...
class EffectiveCharges:
    ...
    def potential(self, r: np.ndarray) -> float:
        ...
        mask  = d > 1e-10
        phi   = np.sum(
            self.charges[mask] * self.bjerrum_length
    ...
    def force_on_charge(self, r: np.ndarray, q: float) -> np.ndarray:
        ...
        coeff   = (self.charges[mask] * self.bjerrum_length * exp_fac
                   * (1.0 / self.debye_length + inv_d) * inv_d)  # (N,)
    ...
    @classmethod
    def from_xml(cls, xml_path: str | Path,
                 debye_length: float = DEFAULT_DEBYE_LENGTH,
                 bjerrum_length: float = BJERRUM_LENGTH) -> "EffectiveCharges":
        """
    ...
    def __repr__(self) -> str:
        return (f"EffectiveCharges({len(self)} charges, "
                f"q_net={self.charges.sum():.2f} e, "
                f"λ_D={self.debye_length:.3f} Å)")


def load_effective_charges(directory:     str | Path,
                            prefix:        str,
                            debye_length:  float = DEFAULT_DEBYE_LENGTH,
                            bjerrum_length: float = BJERRUM_LENGTH
