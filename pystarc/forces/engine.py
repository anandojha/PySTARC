...
try:
    ...
    def _interp(data, origin, inv_sp, point):
        ...
        nx = data.shape[0]; ny = data.shape[1]; nz = data.shape[2]
        ix = (point[0] - origin[0]) * inv_sp[0]
        iy = (point[1] - origin[1]) * inv_sp[1]
        iz = (point[2] - origin[2]) * inv_sp[2]
        i0 = int(math.floor(ix)); j0 = int(math.floor(iy)); k0 = int(math.floor(iz))
        if not (0 <= i0 < nx-1 and 0 <= j0 < ny-1 and 0 <= k0 < nz-1):
            return 0.0
        fx = ix-i0; fy = iy-j0; fz = iz-k0
        return (data[i0,   j0,   k0  ] * (1-fx)*(1-fy)*(1-fz) +
    ...
    def _grad(data, origin, inv_sp, sp, point):
        ...
        for d in range(3):
            h = sp[d] * 0.5
            pp = point.copy(); pp[d] += h
            pm = point.copy(); pm[d] -= h
            g[d] = (_interp(data, origin, inv_sp, pp) -
                    _interp(data, origin, inv_sp, pm)) / (2.0 * h)
        return g
    ...
    def _atom_loop(positions, charges, data, origin, inv_sp, sp, alpha, is_born):
        ...
        cx = 0.0; cy = 0.0; cz = 0.0; n_c = 0
        for i in range(N):
            if abs(charges[i]) > 1e-9:
                cx += positions[i, 0]; cy += positions[i, 1]
                cz += positions[i, 2]; n_c += 1
        if n_c > 0:
            cx /= n_c; cy /= n_c; cz /= n_c
        for i in range(N):
            q = charges[i]
            if abs(q) < 1e-9:
                continue
            ...
            if is_born:
                ...
                coeff   = -q
                energy += q * ph
            fx = coeff * gr[0]; fy = coeff * gr[1]; fz = coeff * gr[2]
    ...
    def _interp(data, origin, inv_sp, point):
        ...
        return float(
            data[i0,j0,k0]*(1-fx)*(1-fy)*(1-fz) + data[i0+1,j0,k0]*fx*(1-fy)*(1-fz) +
            data[i0,j0+1,k0]*(1-fx)*fy*(1-fz)   + data[i0,j0,k0+1]*(1-fx)*(1-fy)*fz +
            data[i0+1,j0+1,k0]*fx*fy*(1-fz)      + data[i0+1,j0,k0+1]*fx*(1-fy)*fz +
            data[i0,j0+1,k0+1]*(1-fx)*fy*fz      + data[i0+1,j0+1,k0+1]*fx*fy*fz)
    ...
    def _grad(data, origin, inv_sp, sp, point):
        ...
        for d in range(3):
            ...
            pp = point.copy(); pp[d] += h
            pm = point.copy(); pm[d] -= h
            g[d] = (_interp(data,origin,inv_sp,pp) - _interp(data,origin,inv_sp,pm))/(2*h)
        return g

    def _atom_loop(positions, charges, data, origin, inv_sp, sp, alpha, is_born):
        N = positions.shape[0]
        force = np.zeros(3); torque = np.zeros(3); energy = 0.0
        mask = np.abs(charges) > 1e-9
        if not mask.any(): return force, torque, energy
...
def _cupy_eval(positions: np.ndarray, charges: np.ndarray,
               g: _Grid, alpha: float, is_born: bool) -> Tuple[np.ndarray, np.ndarray, float]:
    ...
    _CUPY_KERNEL(
        (blocks,), (threads,),
        (pos_gpu, chg_gpu, dat_gpu, ori_gpu, isp_gpu, sp_gpu,
         np.float64(alpha), np.int32(1 if is_born else 0),
         np.int32(nx), np.int32(ny), np.int32(nz),
         forces_gpu, energies_gpu, np.int32(N))
    )
...
class _GridStack:
    ...
    def eval_atoms(self, positions: np.ndarray, charges: np.ndarray,
                   alpha: float, is_born: bool,
                   backend: str) -> Tuple[np.ndarray, np.ndarray, float]:
        """
        ...
        if backend == 'cupy' and _CUPY_KERNEL is not None:
            ...
            for g in self._grids:
                ...
                f, t, e = _atom_loop(sub_pos, sub_chg,
                                      g.data, g.origin, g.inv_spacing,
                                      g.spacing, alpha, is_born)
                total_force  += f
                total_torque += t
                total_energy += e
...
class PySTARCEngine:
    ...
    def _warmup_numba(self):
        ...
        _atom_loop(dummy_pos, dummy_chg, dummy_dat, dummy_ori,
                   dummy_isp, dummy_sp, 0.0, False)

    def __call__(self, mol1: Molecule, mol2: Molecule
        ...
        if self._elec1:
            ...
            for i, (p, q) in enumerate(zip(pos2, chg2)):
                ...
                c2 = pos2[np.abs(chg2) > 1e-9].mean(axis=0) if np.any(np.abs(chg2) > 1e-9) else np.zeros(3)
        ...
        if self._elec2:
            ...
            force -= f; energy += e * 0.5   # avoid double-counting
        ...
        if self._lj_engine is not None:
            ...
            f1, f2, e_lj = self._lj_engine.compute(
                pos1, pos2, type_ids1, type_ids2)
    ...
    def summary(self) -> str:
        ...
        lines.append(f"  Electrostatic grids : {len(self._elec1)} receptor + {len(self._elec2)} ligand")
        lines.append(f"  Born desolvation    : {len(self._born1)} receptor + {len(self._born2)} ligand")
        ...
        if self.backend == 'numpy':
            ...
            lines.append("  [!] Install cupy for GPU:          pip install cupy-cuda12x")
        elif self.backend == 'numba':
            lines.append("  [!] Install cupy for GPU:          pip install cupy-cuda12x")
        return "\n".join(lines)

# Factory 
def load_dx_directory(directory: str | Path,
                      mol1_prefix: str = "receptor",
                      mol2_prefix: str = "ligand",
                      debye_length:      float = 7.858,
                      desolvation_alpha: float = 0.07957747) -> PySTARCEngine:
    """
