...
def ig(s):
    return math.exp(V(1 / s)) / D if s > 0 else 1 / D


Ib, _ = quad(ig, 0, 1 / b, limit=500)
kb = 4 * math.pi / Ib
Iq, _ = quad(ig, 0, 1 / q_esc, limit=500)
kq = 4 * math.pi / Iq
rp = kb / kq
...
Sbq, _ = quad(Si, b, q_esc, limit=500)
Saq, _ = quad(Si, a, q_esc, limit=500)
Ps = Sbq / Saq
Pexact = Ps / (1 - (1 - Ps) * rp)
kon_exact = CONV * kb * Pexact
