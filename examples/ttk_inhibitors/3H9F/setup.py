#!/usr/bin/env python3
"""
PySTARC setup script for TTK kinase / Mps1-IN-2 protein-ligand benchmark.

PDB: 3H9F (Kwiatkowski et al. 2010)
  TTK / MPS1 kinase domain (chain A) + Mps1-IN-2 (type I inhibitor)
  Contains phosphorylated residue(s); handled via phosaa14SB force field.

Experimental k_on: 1.19e6 M^-1 s^-1 (Uitdehaag et al., 2017)

For the TTK-Mps1-IN-2 protein-ligand complex, the receptor is the protein
(ATOM + phospho HETATM records, chain A from residue 515) and the
ligand is the small molecule Mps1-IN-2 (HETATM records, residue 92M).
...
# User settings
PDB_ID                  = "3H9F"                  # PDB accession code
CHAIN_REC               = "A"                     # Protein chain ID in PDB
LIGAND_RESID            = "92M"                   # Ligand residue name in PDB
REC_RESID_MIN           = 515                     # First protein residue (skip His-tag)
LIGAND_NET_CHARGE       = 0                       # Net charge of SB203580 at pH 7
RECEPTOR_RESNAME        = "TTK"                   # Receptor label for XML
LIGAND_RESNAME          = "92M"                   # Ligand label for XML
...
# Reaction criterion (closest polar atom contacts from crystal)
# GLU603 O    <->  N1  (7.0 A cutoff)
# GLY605 O    <->  OBB  (7.0 A cutoff)
# GLY605 N    <->  N1  (7.0 A cutoff)
# LYS529 NZ   <->  OBB  (7.0 A cutoff)
RXN_TARGETS_REC         = [(603, 'O', 'GLU'), (605, 'O', 'GLY'), (605, 'N', 'GLY')]
RXN_TARGETS_LIG         = [('N1',), ('OBB',), ('N1',)]
RXN_CUTOFFS             = [4.5, 4.5, 4.5]    # Per-pair cutoffs (A)
...
APBS_FGLEN              = "144"                   # APBS fine grid length (A)
...
with open(pdb_file) as fh:
    for line in fh:
        ...
        # Phospho residues (TPO/SEP/PTR) appear as HETATM in PDB; convert to ATOM for tleap
        elif line.startswith("HETATM") and len(line) > 21 and line[21] == CHAIN_REC:
            resname = line[17:20].strip()
            if resname in ("TPO", "SEP", "PTR"):
                resid = int(line[22:26].strip())
                if resid >= REC_RESID_MIN:
                    protein_lines.append("ATOM  " + line[6:])
with open("protein.pdb", "w") as fh:
    fh.write(''.join(protein_lines) + 'END\n')
print(f"  protein.pdb: {len(protein_lines)} ATOM lines (including converted TPO/SEP/PTR)")

# Extract ligand HETATM lines + their CONECT records for proper H placement
ligand_lines = []
ligand_serials = set()
with open(pdb_file) as fh:
    for line in fh:
        if line.startswith("HETATM") and line[17:20].strip() == LIGAND_RESID:
            ligand_lines.append(line)
            try:
                ligand_serials.add(int(line[6:11].strip()))
            except ValueError:
                pass
conect_lines = []
with open(pdb_file) as fh:
    for line in fh:
        if line.startswith("CONECT"):
            try:
                if int(line[6:11].strip()) in ligand_serials:
                    conect_lines.append(line)
            except ValueError:
                continue
with open("ligand_raw.pdb", "w") as fh:
    fh.write(''.join(ligand_lines))
    fh.write(''.join(conect_lines))
    fh.write('END\n')
print(f"  ligand_raw.pdb: {len(ligand_lines)} HETATM, {len(conect_lines)} CONECT lines")
...
# Step 3b: Parameterize ligand with OpenEye (AM1-BCC charges) + antechamber (GAFF2 atom types)
print(f"\nStep 3b: OpenEye AM1-BCC charges + antechamber GAFF2 atom typing (net charge = {LIGAND_NET_CHARGE})")
from openeye import oechem, oequacpac
ifs = oechem.oemolistream("ligand_raw.pdb")
ifs.SetFormat(oechem.OEFormat_PDB)
mol = oechem.OEMol()
oechem.OEReadMolecule(ifs, mol)
oechem.OEAddExplicitHydrogens(mol)
if not oequacpac.OEAssignCharges(mol, oequacpac.OEAM1BCCCharges()):
    print("  ERROR: OpenEye OEAssignCharges failed.")
    sys.exit(1)
net_q = sum(a.GetPartialCharge() for a in mol.GetAtoms())
print(f"  OpenEye AM1-BCC charges assigned: net = {net_q:+.4f} e ({mol.NumAtoms()} atoms)")
ofs = oechem.oemolostream("ligand_oe.mol2")
ofs.SetFormat(oechem.OEFormat_MOL2)
oechem.OEWriteMolecule(ofs, mol)
ofs.close()
# antechamber for GAFF2 atom typing (preserves OpenEye charges from input MOL2)
r = subprocess.run(
    f"antechamber -i ligand_oe.mol2 -fi mol2 -o ligand.mol2 -fo mol2 "
    f"-at gaff2 -nc {LIGAND_NET_CHARGE}",
    shell=True, capture_output=True, text=True
)
if not os.path.exists("ligand.mol2"):
    print(f"  ERROR: antechamber failed.\n  {r.stderr[:500]}")
    sys.exit(1)
print("  ligand.mol2 created (OpenEye AM1-BCC + GAFF2 atom types)")
...
print(f"  receptor.pqr (TTK): {len(rec_lines)} atoms, Q={rec_charge:+.1f} e")
# Ligand PQR
...
print(f"  ligand.pqr (Mps1-IN-2): {len(lig_lines)} atoms, Q={lig_charge:+.1f} e")
...
def find_rec_atoms(pqr_file, targets, protein_pdb="protein.pdb"):
    """Find receptor atom line numbers matching (PDB_resid, name, resname) targets.
    
    The PQR may have residues renumbered (1..N) by tleap+ambpdb. We map through
    protein.pdb (original numbering preserved) to find the sequential position 
    of each target residue, then look up that position in the PQR.
    """
    # Build PDB resid -> sequential 1-based position
    seen = []
    with open(protein_pdb) as fh:
        for line in fh:
            if line.startswith("ATOM"):
                ...
                if r not in seen:
                    seen.append(r)
    pdb_to_seq = {r: i+1 for i, r in enumerate(seen)}
    # Walk through PQR, find matches
    found = {}
    atom_num = 0
    with open(pqr_file) as fh:
        for line in fh:
            if line.startswith("ATOM"):
                atom_num += 1
                parts = line.split()
                resi_pqr = int(parts[4])
                name = parts[2]
                resn = parts[3]
                for tresi_pdb, tname, tresn in targets:
                    tresi_seq = pdb_to_seq.get(tresi_pdb)
                    if tresi_seq is None:
                        continue
                    if resi_pqr == tresi_seq and name == tname and resn == tresn:
                        found[(tresi_pdb, tname)] = atom_num
                        print(f"  {pqr_file} atom {atom_num}: {name} {resn}{tresi_pdb} (PQR resid {resi_pqr})")
    return found

def find_lig_atoms(pqr_file, targets, ligand_pdb="ligand_raw.pdb"):
    """Find ligand atom line numbers matching original PDB (name,) targets.
    
    OpenEye + antechamber renames atoms (e.g., 'NBC' -> 'N7', 'N9' -> 'N4')
    but preserves heavy-atom order from the input PDB. We look up the original
    atom name in ligand_raw.pdb to find its 1-based position, which equals
    the corresponding PQR atom index.
    """
    name_to_pos = {}
    pos = 0
    ...
    for (tname,) in targets:
        if tname in name_to_pos and tname not in found:
            found[tname] = name_to_pos[tname]
            print(f"  {pqr_file} atom {name_to_pos[tname]}: {tname} (from original PDB order)")
    return found
...
print(f"  Receptor:     TTK ({len(rec_lines)} atoms, Q={rec_charge:+.1f} e)")
print(f"  Ligand:       Mps1-IN-2 ({len(lig_lines)} atoms, Q={lig_charge:+.1f} e)")
